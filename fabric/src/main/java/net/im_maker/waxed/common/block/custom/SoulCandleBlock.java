...
public class SoulCandleBlock extends CandleBlock {
    ...
    public SoulCandleBlock(Properties pProperties, ResourceLocation particle) {
        super(pProperties);
    ...
    public void animateTick(BlockState pState, Level pLevel, BlockPos pPos, RandomSource pRandom) {
        if (pState.getValue(LIT)) {
            this.getParticleOffsets(pState).forEach((vec3) -> {
                addParticlesAndSound(pLevel, particle, vec3.add(pPos.getX(), pPos.getY(), pPos.getZ()), pRandom);
            });
        }
    }
